...
function makeThree() {
  window.localStorage.setItem("three", "name=John, age=25");
...
function showEleven() {
  const elevenJSON = localStorage.getItem("arrayEleven");
  arrayEleven = JSON.parse(elevenJSON);
...
const showNine = () => {
  ...
  if (arrayNineString) {
    const resultElement2 = document.querySelector(".practicum14");
    for (const key in arrayNine) {
      const value = arrayNine[key];
      resultElement2.innerHTML += `${key} ${value}<br>`;
    }
  } else {
...
document.querySelector(".b-14").onclick = showNine;
...
const showEight = () => {
  const eightJSON = localStorage.getItem("objectEight");
  objectEight = JSON.parse(eightJSON);
...
const showSeven = () => {
  const objectSevenJSON = localStorage.getItem("objectSeven");
  objectSeven = JSON.parse(objectSevenJSON);
...
const showSix = () => {
  const objectSixJSON = localStorage.getItem("objectSix");
  objectSix = JSON.parse(objectSixJSON);
...
const removeFromCart = () => {
  ...
  if (cartItems.length > 0) {
    ...
    console.log("Последний продукт удален из корзины");
  } else {
...
const updateCartCount = () => {
  ...
  resultElement.innerHTML = `Количесто товаров  корзине: ${cartItems.length}`;
};
